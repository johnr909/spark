<?php
			...
			<fieldset>
				...
				<div>
					<?php 
						 ...
						 */ 
					?>
					<label for="myplugin_media"><?php _e( 'Field Label', 'reviews' )?></label><br>
	...
	function myplugin_load_admin_scripts( $hook ) {
		global $typenow;
		if( $typenow == 'reviewer_data_metabox' ) {
			wp_enqueue_media();

			// Registers and enqueues the required javascript.
			wp_register_script( 'meta-box-image', get_template_directory_uri() . '/inc/media-mgr/myplugin-media.js', array( 'jquery' ) );
			wp_localize_script( 'meta-box-image', 'meta_image',
				array(
					'title' => __( 'Choose or Upload Media', 'reviews' ),
					'button' => __( 'Use this media', 'reviews' ),
				)
			);
