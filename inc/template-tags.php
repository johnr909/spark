...
if ( ! function_exists( 'spark_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
    function spark_posted_on() {
...
if ( ! function_exists( 'spark_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function spark_entry_footer() {
	// Hide category and tag text for pages.
	if ( 'post' === get_post_type() ) {
		/* translators: used between list items, there is a space after the comma */
		$categories_list = get_the_category_list( esc_html__( ', ', 'spark' ) );
		if ( $categories_list && spark_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'spark' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'spark' ) );
		if ( $tags_list ) {
			printf( ' | <span class="tags-links">' . esc_html__( 'Tagged %1$s', 'spark' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
 ...
 * @return bool
 */
function spark_categorized_blog() {
	...
	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so spark_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so spark_categorized_blog should return false.
		return false;
	}
}
 ...
 */
function spark_category_transient_flusher() {
...
endif; // ends check for spark_comment()
